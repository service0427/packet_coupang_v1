...
from work.request import make_request, parse_response_cookies, timestamp
from extractor.detail_extractor import extract_product_detail


def click_product(product_info, search_url, cookies, tls_profile, proxy, verbose=True, save_html=False):
    """상품 클릭 (상세 페이지 요청)
    ...
    try:
        resp = make_request(
            full_url, cookies, tls_profile, proxy,
            referer=search_url
        )
